...
const RMK_CHANGELOG = [
  {
    ...
    items: [
      'Фикс: при поэкземплярном перемещении через акт инвентаризации теперь меняется не только склад экземпляра, но и его вариант (числовой остаток на складе-получателе больше не зависает на 0).',
      'Вариант склада-получателя подбирается по размеру/характеристике — новые «левые» варианты не плодятся; если варианта нет — создаётся идентичный.',
      'Откат акта возвращает и склад, и исходный вариант, пересчитывая оба варианта по факту.',
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      ...
      'Причина: позиции группировались по характеристике (charC1Ref), а у нативных товаров она пустая (null) — и все они сливались.',
      'Теперь группировка строго по variant_id (точный ключ варианта) — разные товары больше не путаются.',
      'На списание и цены это не влияло задним числом — чек всегда списывал конкретные отсканированные штрихкоды.'
    ]
  },
  {
