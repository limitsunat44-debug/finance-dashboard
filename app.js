...
function updateDashboard() {
    ...
    // Auto-set report date range to current month and generate report
...
function updateDailySalesChart() {
    // Chart removed - reports moved to dashboard
}
...
async function addExpense(event) {
    ...
    // Refresh table and update dashboard
    loadExpensesTable();
    updateDashboard();
...
function generateReport() {
    ...
    const totalProfit = totalRevenue * 0.3; // Net profit (30% margin)
    ...
    // Calculate current total debt to suppliers
    ...
    // Update additional metric cards
    ...
    // Generate expenses by salon chart
...
function generateExpensesBySalonChart(expenses) {
    ...
    expenses.forEach(expense => {
        const salon = expense.salon || 'Общие';
...
function generateReportCharts(sales, expenses) {
    // Charts removed - they were causing errors because canvas elements don't exist
    console.log('Charts generation skipped');
}
