...
// ═══════════════════════════════════════════════════════════════
// УЛУЧШЕННАЯ СИСТЕМА СИНХРОНИЗАЦИИ JSONBIN
// ═══════════════════════════════════════════════════════════════
...
function processSaveQueue() {
    ...
    // Запускаем сохранение с задержкой (debouncing)
    saveTimeout = setTimeout(async () => {
        await executeSave();
    }, SAVE_DEBOUNCE_DELAY);
}

// Основная функция выполнения сохранения
async function executeSave(retryCount = 0) {
    ...
    if (timeSinceLastSave < MIN_SAVE_INTERVAL) {
        // Слишком рано для следующего сохранения, откладываем
    ...
    try {
        const response = await fetch(JSONBIN_CONFIG.baseUrl, {
            ...
            headers: {
                ...
                'X-Master-Key': JSONBIN_CONFIG.masterKey,
                'X-Bin-Versioning': 'false' // Отключаем версионирование для ускорения
            },
        ...
        // Разрешаем все промисы в очереди
        ...
        if (shouldRetry && retryCount < MAX_RETRIES) {
            ...
            // Если конфликт, сначала перезагружаем данные
            if (error.message === 'CONFLICT') {
                console.log('⚠ Conflict detected, reloading data before retry...');
                await loadDataSilently(); // Тихая перезагрузка без UI обновлений
            }
...
// Тихая загрузка данных без обновления UI (для разрешения конфликтов)
async function loadDataSilently() {
    try {
        ...
        if (response.ok) {
            const result = await response.json();
            if (result.record) {
                // Объединяем данные с сервера с локальными изменениями
                appData = { ...result.record, ...appData };

                // Убеждаемся что exchangeRate существует
                if (!appData.exchangeRate || appData.exchangeRate <= 0) {
                    appData.exchangeRate = 10.50;
                }

                console.log('✓ Data reloaded silently');
            }
        }
    } catch (error) {
...
// Функция для показа индикатора синхронизации
function showSyncIndicator(status = 'syncing', message = '') {
    const indicator = document.getElementById('syncIndicator');
    if (!indicator) return; // Если индикатор не добавлен в HTML, пропускаем
    ...
    if (status !== 'error') {
        ...
        // Для ошибок скрываем через 5 секунд
        setTimeout(() => {
            indicator.classList.remove('visible');
        }, 5000);
    }
